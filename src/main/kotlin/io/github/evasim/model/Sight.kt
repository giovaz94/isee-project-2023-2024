...
interface Sight {
    ...
    /** Updates the sight's pose based on the given one. */
    fun update(pose: Pose) = update(pose.first, pose.second)

    /** Updates the sight's pose based on the given position and direction. */
    fun update(position: Position2D, direction: Vector2D)

    /** The sight's factory methods. */
    companion object {
        /** Creates a new sight with the specified shape and initial pose. */
        operator fun invoke(shape: Shape, initialPosition: Position2D, direction: Vector2D): Sight =
            SightImpl(shape at Pose(initialPosition, direction))
    }
}

internal data class SightImpl(override val visibilityArea: Placed<out Shape>) : Sight {

    override fun update(position: Position2D, direction: Vector2D) = visibilityArea.update(position, direction)
